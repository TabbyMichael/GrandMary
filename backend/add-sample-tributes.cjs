...
// Sample tributes data (matching actual tributes table schema)
...
async function addSampleTributes() {
  try {
    console.log('🌹 Adding sample tributes to the database...');
    ...
    data.forEach((tribute, index) => {
      const name = tribute.author_name || 'Unknown';
      const relationship = tribute.author_relationship || 'Friend';
    ...
    console.log(`\n✅ Verification: ${verifyData.length} approved tributes now in database`);
